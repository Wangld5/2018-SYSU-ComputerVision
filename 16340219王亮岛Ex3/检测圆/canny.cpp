...
cannyImg::cannyImg(string filename, int sigma, int low, int high){
	...
	vector<vector<double>> filter = createFilter(5, 5, sigma);
	...
	sFiltered = sobel();
	non = nonMaxSupp();
	thres = threshold(non, low, high);
    string gray = "gray_"+filename;
    string gauss = "gauss_"+filename;
    string sobels = "sobel_"+filename;
    string nms = "nonMaxSupp_"+filename;
    string thre = "thre_"+to_string(sigma)+"_"+to_string(low)+"_"+to_string(high)+"_"+filename;
    string links = "link_"+to_string(sigma)+"_"+to_string(low)+"_"+to_string(high)+"_"+filename;
 
	
    // img.display("Original");
    // grayscaled.display("GrayScaled");
    // //grayscaled.save(gray.c_str());
    // gFiltered.display("Gaussian Blur");
    // //gFiltered.save(gauss.c_str());
    // sFiltered.display("Sobel Filtered");
    // //sFiltered.save(sobels.c_str());
    // non.display("Non-Maxima Supp");
    //non.save(nms.c_str());
    // thres.display("Final");
    // thres.save(thre.c_str());
     link = linkEdge(thres, low, high);
     link.display("link");
     link.save(links.c_str());
...
CImg<unsigned char> cannyImg::linkEdge(CImg<unsigned char>& imageIn, int low, int high){
            ...
            while(!possiblePoint.empty()){
                ...
                for(int x=temp.first-1; x<=temp.first+1; x++){
                    for(int y=temp.second-1; y<=temp.second+1; y++){
                        if(x<0 || y<0 || x>=imageIn.width() || y>=imageIn.height()){//这里要注意的是处于图片四个边上也算出界了。
                            continue;
                        }
                        if(thres(x, y) > low && thres(x, y) < high && flag[x][y] == 0){
    ...
    cout<<"find strong edge finish"<<endl;
    ...
    cimg_forXY(imageIn, i, j){
        if(finalEdge(i, j) == 255){
            ...
            while(!possiblePoint.empty()){
                ...
                for(int x=temp.first-1; x<=temp.first+1; x++){
                    for(int y=temp.second-1; y<=temp.second+1; y++){
                        if(x<0 || y<0 || x>=imageIn.width() || y>=imageIn.height()){//这里要注意当等于边缘
                            continue;
                        }
